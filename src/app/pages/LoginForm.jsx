...
import { sharedLogin } from '../redux/shared/authActions'

export default function Login() {
    const [formData, setFormData] = useState({ email: '', password: '', role: 'doctor' }) // Initialize role
    ...
    const handleLogin = (e) => {
        ...
        const role = e.target.role.value; // Get the selected role from the form

        // Dispatch sharedLogin action
        dispatch(sharedLogin(email, password, role));
    }

    // Handle redirection based on user type (doctor, patient, clinic)
    useEffect(() => {
        if (isLoggedIn) {
            switch (userType) {
                case 'doctor':
