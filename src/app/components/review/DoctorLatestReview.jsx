...
const DoctorLatestReview = () => {
  ...
  useEffect(() => {
    const fetchDoctorsWithReviews = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/review/doctors-with-latest-reviews`);  
          ...
          {doctors.slice(0, 3).map((doctor) => (
            <div key={doctor._id} className="doctor-card mt-4 flex gap-4">
              <div className="flex-shrink-0">
                  ...
                  <Link href={`/profile-info/${doctor._id}`} className='w-full'>
                    <img
                  ...
                  <div className="mt-2 bg-gray-100  rounded-md">
                    ...
                    {doctor.latestReview.comment && (
                        ...
                        {truncateByWords(doctor.latestReview.comment, 20).isTruncated && (
                            <span className="text-gray-900 font-medium text-sm cursor-pointer ml-2 hover:underline" onClick={() => router.push(`/profile-info/${doctor._id}`)}>
                            More...
                            </span>
                        )}
                        </p>
